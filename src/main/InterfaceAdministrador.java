...
public class InterfaceAdministrador {
	...
	private static void editarCategoria(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras, int index) {
        ...
        try {
            ...
            if (result == JOptionPane.OK_OPTION) {
            	...
            	// Após editar a categoria na lista, é necessário atualizar a categoria nos veículos que a utilizam
	...
	private static void excluirCategoria(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras, int index) {
		...
		int dialogResult = JOptionPane.showConfirmDialog(null, "Você tem certeze que deseja excluir a categoria '" + categorias.get(index).getCodigo() + " - " + categorias.get(index).getNome() + "'?", "ATENÇÃO", dialogButton);
				...
				for(int i = 0; i < locadoras.size(); i++) {
					for(int j = 0; j < locadoras.get(i).getVeiculos().size(); j++) {
						if(locadoras.get(i).getVeiculos().get(j).getCategoria().getCodigo() == categorias.get(index).getCodigo()) {
							...
							msg += "Veículo: " + locadoras.get(i).getVeiculos().get(j).getCodigo() + " - " + locadoras.get(i).getVeiculos().get(j).getMarca() + " - " + locadoras.get(i).getVeiculos().get(j).getModelo() + " - " + locadoras.get(i).getVeiculos().get(j).getAno() + "\n";
						}
					}
				}
	...
	private static void cadastrarLocadora(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras) {
        ...
        try {
        	int result = JOptionPane.showConfirmDialog(null, panel, "Cadastrar Locadora",
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
            
            if (result == JOptionPane.OK_OPTION) {
            	Localizacao loc = new Localizacao(campo_endereco.getText(), campo_bairro.getText(), campo_cidade.getText(), campo_estado.getText());
	...
	private static void editarLocadora(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras, int index) {
        ...
        try {
        	int result = JOptionPane.showConfirmDialog(null, panel, "Editar Locadora",
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
	...
	private static void excluirLocadora(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras, int index) {
		int dialogButton = JOptionPane.YES_NO_OPTION;
		int dialogResult = JOptionPane.showConfirmDialog(null, "Você tem certeze que deseja excluir a locadora '" + locadoras.get(index).getNome() + "'?", "ATENÇÃO", dialogButton);
		
		if(dialogResult == JOptionPane.YES_OPTION){
			JOptionPane.showMessageDialog(null, "Locadora excluída com sucesso!", null, JOptionPane.INFORMATION_MESSAGE );
	...
	private static void visualizarLocadora(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras, int index) {
		...
		for(int i = 0; i < locadoras.get(index).getVeiculos().size(); i++) {
			nomes[i] = locadoras.get(index).getVeiculos().get(i).getCodigo() + " - " + locadoras.get(index).getVeiculos().get(i).getMarca() + " - " + locadoras.get(index).getVeiculos().get(i).getModelo() + " - " + Integer.toString(locadoras.get(index).getVeiculos().get(i).getAno());
		}
	...
	private static void cadastrarVeiculo(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras, int index_locadora) {
        ...
        try {
        	int result = JOptionPane.showConfirmDialog(null, panel, "Cadastrar Veículo",
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        	
        	if (result == JOptionPane.OK_OPTION) {
	        	if(campo_categoria.getSelectedItem() != null) {
	        		Veiculo v = new Veiculo(campo_marca.getText(), campo_modelo.getText(), Integer.parseInt(campo_ano.getText()), campo_acessorios.getText(), Double.parseDouble(campo_preco.getText()), categorias.get(campo_categoria.getSelectedIndex()));
	        		locadoras.get(index_locadora).addVeiculo(v);
                    JOptionPane.showMessageDialog(null, "Veículo cadastrado com sucesso!");
	        	}else {
	        		JOptionPane.showMessageDialog(null, "É necessário escolher uma categoria para cadastrar!", "Erro", JOptionPane.ERROR_MESSAGE );
	        	}
        	}
        }catch(Exception e) {
        	JOptionPane.showMessageDialog(null, "Ocorreu uma exceção.");
	...
	private static void editarVeiculo(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras, int index_locadora, int index) {
        ...
        JTextField campo_ano = new JTextField(Integer.toString(locadoras.get(index_locadora).getVeiculos().get(index).getAno()));
        
        JTextField campo_acessorios = new JTextField(locadoras.get(index_locadora).getVeiculos().get(index).getAcessorios());
        
        JTextField campo_preco = new JTextField(Double.toString(locadoras.get(index_locadora).getVeiculos().get(index).getPreco()));
		...
		for(int i = 0; i < categorias.size(); i++) {
			...
			if(categorias.get(i).getCodigo() == locadoras.get(index_locadora).getVeiculos().get(index).getCategoria().getCodigo()) {
				cat_index = i;
			}
		}
        JComboBox<String> campo_categoria = new JComboBox<>(categorias_nomes);
        ...
        try {
        	int result = JOptionPane.showConfirmDialog(null, panel, "Editar Veículo",
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        	...
        	if (result == JOptionPane.OK_OPTION) {
	        	if(campo_categoria.getSelectedItem() != null) {
	        		...
	        		locadoras.get(index_locadora).getVeiculos().get(index).setAno(Integer.parseInt(campo_ano.getText()));
	        		locadoras.get(index_locadora).getVeiculos().get(index).setAcessorios(campo_acessorios.getText());
	        		locadoras.get(index_locadora).getVeiculos().get(index).setPreco(Double.parseDouble(campo_preco.getText()));
	        		locadoras.get(index_locadora).getVeiculos().get(index).setCategoria(categorias.get(campo_categoria.getSelectedIndex()));
	        		
                    JOptionPane.showMessageDialog(null, "Veículo editado com sucesso!");
	        	}else {
	        		JOptionPane.showMessageDialog(null, "É necessário escolher uma categoria!", "Erro", JOptionPane.ERROR_MESSAGE );
	        	}
        	}
        }catch(Exception e) {
        	JOptionPane.showMessageDialog(null, "Ocorreu uma exceção.");
	...
	private static void excluirVeiculo(ArrayList<Categoria> categorias, ArrayList<Locadora> locadoras, int index_locadora, int index) {
		int dialogButton = JOptionPane.YES_NO_OPTION;
		int dialogResult = JOptionPane.showConfirmDialog(null, "Você tem certeze que deseja excluir o veículo '" + locadoras.get(index_locadora).getVeiculos().get(index).getMarca() + " - " + locadoras.get(index_locadora).getVeiculos().get(index).getModelo() + "'?", "ATENÇÃO", dialogButton);
